...
const ProjectCard = ({ title, url, image, description, source }) => {
  return (
    <Card sx={{ width: '22vw', borderRadius: '12px', height: '42vh' }}>
      ...
      <CardActions>
        <button className="project-btn" onClick={() => window.open(url, "_blank")}>Link</button>
        <button className="project-btn" onClick={() => window.open(source, "_blank")}>Source Code</button>
      </CardActions>
    </Card>
  )
}
